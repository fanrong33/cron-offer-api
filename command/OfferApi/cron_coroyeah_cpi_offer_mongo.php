<?php
/**
 * 每20分钟拉一次coroyeah cpi api 任务计划（MongoDB版）
 * 
...
// 32,02  *  *  *  * root /usr/bin/php /data/www/command_jc/OfferApi/cron_coroyeah_cpi_offer_mongo.php 2>&1 >> /dev/null &
error_reporting(0);
set_time_limit(0);
...
require_once dirname(__FILE__).'/../Core/Coroyeah.class.php';
...
$coroyeah = new Coroyeah($config['COROYEAH_API_TOKEN']);
$aff_id     = $config['COROYEAH_AFF_ID'];
$network_id = 69;
...
function get_offer_list_via_page($page, $limit, $coroyeah, &$offer_list, &$api_request_status){
    $params = array(
        // 'page'  => $page,
        // 'limit' => $limit,
    );
    $json = $coroyeah->get_offer($params);
    if($json['status'] == 0){
        ...
        if($total_pages > $page){
            unset($json);
            $page++;
            get_offer_list_via_page($page, $limit, $coroyeah, $offer_list, $api_request_status);
        }
    }else{
